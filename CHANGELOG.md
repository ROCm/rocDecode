...
* A new sample app, videodecoderaw, which uses the bitstream reader instead of FFMPEG demuxer to get picture data.
* More CTests: VP9 test and tests on video decode raw sample.
* A new sample app, videodecodepicfiles, which reads individual picture data from a group of files with each file containing bitstream of a coded picture.
